...
// Self-contained. This endpoint is the "digest" engine; a scheduler (Vercel
// cron once/day, or a free external cron every 4h) just pokes it. It's
// idempotent: it only emails when a game's roster changed since the last send,
// or once on the eve of game day, and never on game day itself. Safe to call
...
export default async function handler(req, res) {
  ...
  // Upcoming games only. game_date > today excludes BOTH the past and today
  // (game day), so "never email on game day" falls out for free.
  const { data: games, error } = await supabaseAdmin
    .from('games')
    .select('id, location, game_date, first_tee_time, tee_times, capacity')
    .gt('game_date', today);
...
async function processGame(game, tomorrow) {
  const isEve = game.game_date === tomorrow;

  // Current roster, in registration order (used for waitlist order too).
  const { data: regs } = await supabaseAdmin
    .from('registrations')
    .select('golfer_id, status, registered_at, golfers ( name )')
    .eq('game_id', game.id)
  ...
  // Deterministic signature of roster state -> "did anything change?"
  const signature = (regs || [])
    .map((r) => `${r.golfer_id}:${r.status}`)
    .sort()
    .join('|');
  ...
  // Don't fire an empty first digest mid-week (avoids noise). On the eve, the
  // guaranteed send happens regardless.
  ...
  // Recipients = ALL invitees (registered, waitlisted, and not-yet-responded).
  const { data: invitees } = await supabaseAdmin
    .from('game_invitees')
    .select('golfers ( email )')
    .eq('game_id', game.id);
  const recipients = (invitees || []).map((i) => i.golfers?.email).filter(Boolean);
  if (recipients.length === 0) return { game_id: game.id, sent: false, reason: 'no-invitees' };

  // 7-day forecast, best-effort. Weather never blocks the roster update.
  let weather = null;
  try {
    if (APP_URL) {
      const wr = await fetch(`${APP_URL}/api/weather?location=${encodeURIComponent(game.location)}&days=7`);
  ...
  const subject = `Golf ${formatDate(game.game_date)} - ${registered.length}/${game.capacity} in`;
  const html = digestHtml({ game, registered, waitlisted, weather });
...
function digestHtml({ game, registered, waitlisted, weather }) {
  const spots = Math.max(0, game.capacity - registered.length);
  const regList = registered.length
    ? registered.map((r) => `<li>${escapeHtml(r.golfers?.name || 'Golfer')}</li>`).join('')
  ...
  const waitBlock = waitlisted.length
    ? `<p style="margin:16px 0 4px;font-weight:600;">Waitlist</p>
       <ol style="margin:0 0 0 18px;padding:0;">${waitlisted
         .map((r) => `<li>${escapeHtml(r.golfers?.name || 'Golfer')}</li>`)
         .join('')}</ol>`
    : '';
  ...
  if (weather?.days?.length) {
    const rows = weather.days
      ...
      .join('');
    forecast = `<p style="margin:22px 0 6px;font-weight:600;">7-day forecast - ${escapeHtml(weather.courseName || game.location)}</p>
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
    <table role="presentation" width="520" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:10px;max-width:520px;width:100%;">
      ...
      <tr><td style="padding:24px;color:#1a2e1a;font-size:15px;line-height:1.55;">
        <p style="margin:0 0 2px;font-size:17px;font-weight:600;">${escapeHtml(formatDate(game.game_date))}</p>
        <p style="margin:0 0 12px;color:#4a5a4a;">${escapeHtml(game.location)}</p>
        <p style="margin:0 0 4px;"><strong>${registered.length} registered</strong> &middot; ${spots} spot${spots === 1 ? '' : 's'} available</p>
        <p style="margin:0 0 12px;color:#4a5a4a;">First tee time ${escapeHtml(formatTime(game.first_tee_time))} - plan to arrive for the first group.</p>
